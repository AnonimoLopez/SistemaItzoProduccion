...
<li class="">
						<a href="#" class="dropdown-toggle">
							...
							';
	$menuprincipalHeader2=		'

								<span class="badge badge-primary">';

$menuprincipalHeader3='</span>
							</span>
...
if ($result) {
	while($fila = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)){
		...
		if ($fila['tipo'] == "MODULO") {
			if ($moduloanterior == "") {
				$nameGroup = $fila['Descripcion'];
				$menu.=$menuprincipalHeader.$nameGroup.$menuprincipalHeader2.$fNo.$menuprincipalHeader3;
				$abierto = true;	
			}
            ...
            if ($fila['No'] == 0){
				...
				if ($moduloanterior <> $fila['Descripcion'] and $moduloanterior <> ""){
					$nameGroup = $fila['Descripcion'];
					$menu.=$menuprincipalFooter;
					$menu.='</ul>'.$menuprincipalHeader.$nameGroup.$menuprincipalHeader2.$fNo.$menuprincipalHeader3;
				}
				
			}

			$moduloanterior = $fila['Descripcion'];
		}	else{
			$id=" id ='".$fila['form']."'";
			if ($no == 0){
...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="static/notificaciones/build/toastr.min.css" />
		<link rel="stylesheet" href="static/notificaciones/build/toastr.css" />
				...
				<div class="main-content-inner">
					<div class="page-content">
						...
						<div class="panel panel-info">
  							...
  							<div class="panel-footer">Ninguna Aviso Importante</div>
						</div>
...
<![endif]-->
  ...
  <script type="text/javascript" src="static/pacejs/js/pace.js"></script>
		...
		$('a').click( function(e) {
				...
				if ($(this).attr('href') != '#' &&  $(this).attr('id') == '0') {
					e.preventDefault(); 
					go($(this).attr('href'));
					return false;
				}else if ($(this).attr('href') != '#' &&  $(this).attr('id') == '1'){
					e.preventDefault(); 
					window.open($(this).attr('href'),'Reporte','location=no,menubar=no,status=no,toolbar=no');
					return false;
				}
			  //return true; 
			}
